...
function isDateInPeriod(date, period) {
  const a = Date.parse(new Date(period.start)) <= Date.parse(new Date(date));
  const b = Date.parse(new Date(date)) <= Date.parse(new Date(period.end));
  return a && b;
}
...
function formatDate(date) {
  const sameDate = new Date(Date.parse(date)).toISOString();
  return new Date(sameDate).toLocaleString('en-US', { timeZone: 'UTC' });
}
...
function getWeekNumberByDate(date) {
  ...
  const quantityWeek = Math.ceil(
    (januaryFirst.getDay() +
      Math.floor((date - januaryFirst) / 1000 / 60 / 60 / 24) +
      2) /
      7
  );
  return quantityWeek;
...
function getNextFridayThe13th(date) {
  ...
  if (oldDay > 13 && month < 11) {
    ...
    day.setMonth(month - 11);
    day.setFullYear(year + 1);
  }
  for (let i = month; i < 13; i += 1) {
    day.setUTCMonth(i);
    if (day.getDay() === 5) {
      break;
    }
